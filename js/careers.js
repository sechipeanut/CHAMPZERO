    ...
    try {
        ...
        querySnapshot.forEach(doc => {
            ...
            div.innerHTML = `
                ...
                <a href="/contact?subject=Application for ${encodeURIComponent(job.title)}" class="w-full sm:w-auto px-6 py-2.5 text-center text-black bg-[var(--gold)] font-bold rounded-lg hover:bg-yellow-400 transition-colors shadow-md">
                    Apply Now
                </a>
            `;
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            if (adminEmails.includes(user.email)) {
                adminArea.innerHTML = `
                    <a href="/admin" class="inline-flex items-center gap-2 bg-red-600/90 hover:bg-red-600 text-white px-6 py-2 rounded-full font-bold shadow-lg transition-transform hover:scale-105 backdrop-blur-md border border-red-500/50">
